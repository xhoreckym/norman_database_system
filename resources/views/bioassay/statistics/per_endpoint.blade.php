...
@section('page-title', 'Records by Endpoint')
@section('page-subtitle', 'Number of records per bioassay endpoint')
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <div class="text-2xl font-bold text-gray-900">{{ number_format($totalEndpoints, 0, '.', ' ') }}</div>
        </div>
        <div>
        ...
        <div>
          <div class="text-sm text-gray-600">Average Records per Endpoint</div>
          <div class="text-2xl font-bold text-gray-900">
            {{ number_format(collect($data)->sum(fn($item) => $item['count']) / max($totalEndpoints, 1), 1, '.', ' ') }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @foreach(collect($data)->sortByDesc(fn($item) => $item['count']) as $endpoint => $info)
              <tr>
