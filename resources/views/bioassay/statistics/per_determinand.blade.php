...
@section('page-title', 'Records by Determinand')
@section('page-subtitle', 'Number of records per main determinand')
  ...
  @else
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <div class="text-sm text-gray-600">Total Determinands</div>
          <div class="text-2xl font-bold text-gray-900">{{ number_format($totalDeterminands, 0, '.', ' ') }}</div>
        </div>
        <div>
        ...
        <div>
          <div class="text-sm text-gray-600">Average Records per Determinand</div>
          <div class="text-2xl font-bold text-gray-900">
            {{ number_format(collect($data)->sum(fn($item) => $item['count']) / max($totalDeterminands, 1), 1, '.', ' ') }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @foreach(collect($data)->sortByDesc(fn($item) => $item['count']) as $determinand => $info)
              <tr>
