<div>
  ...
  <input type="text" wire:model.live.debounce.300ms="search" name="searchSubstanceString" id="searchSubstanceString"
    ...
    placeholder="Search for substances with ecotox data...">
  <div class="mt-4">
  ...
  <div>
    @if ($resultsAvailable == true)
      <div class="mt-2 p-2 bg-lime-50 border-l-4 border-lime-500 text-lime-800 text-sm">
        <p>Select a substance and click <span class="font-semibold">Select & Search</span> to view its ecotoxicology
          data.</p>
      </div>
    @endif
          ...
          <div class="block p-2 hover:bg-gray-50 border-b border-gray-100 last:border-b-0">
            <label class="flex items-center cursor-pointer">
              ...
              @if (isset($result->ecotox_record_count))
                <span
                  class="ml-2 bg-green-100 px-2 py-1 rounded-full text-xs font-medium">{{ $result->ecotox_record_count }}
                  records</span>
              @endif
            </label>
          </div>
