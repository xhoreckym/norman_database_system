<div>
    ...
    @if (count($selectedSubstances) > 0)
      ...
      <div class="flex flex-wrap gap-2">
        @foreach ($selectedSubstances as $substance)
          <div
            class="flex items-center px-4 py-2 rounded-lg bg-lime-50 border border-lime-200 shadow-sm group hover:bg-lime-100 transition duration-150 ease-in-out">
            ...
            @if (isset($substance['pnec3_record_count']))
              <div class="ml-3 flex items-center gap-1 px-2 py-1 rounded-full bg-lime-200 text-lime-800">
                ...
                <span class="text-xs font-medium">{{ $substance['pnec3_record_count'] }} records</span>
              </div>
            @endif

            <button type="button" wire:click="removeSubstance({{ $substance['id'] }})"
              class="ml-3 text-lime-800 hover:text-red-600 focus:outline-none group-hover:text-red-600 transition duration-150 ease-in-out">
  ...
  <input type="text" wire:model.live.debounce.300ms="search" name="searchSubstanceString" id="searchSubstanceString"
    class="w-full px-4 py-2 border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent transition duration-200 ease-in-out"
    placeholder="Search for substances with PNEC data...">
  <div class="mt-4">
  ...
  <div class="flex flex-col md:flex-row justify-between text-sm text-gray-600 mt-2">
    <span>The search is limited to 30 substances with PNEC data</span>
  ...
  <div>
    @if ($resultsAvailable == true)
      <div class="mt-2 p-2 bg-lime-50 border-l-4 border-lime-500 text-lime-800 text-sm">
        <p>Select a substance and click <span class="font-semibold">Select & Search</span> to view its PNEC data.</p>
      </div>
    @endif
        ...
        @foreach ($results as $result)
          <div class="block p-2 hover:bg-gray-50 border-b border-gray-100 last:border-b-0">
            <label class="flex items-center cursor-pointer">
              <input wire:model.live="selectedSubstanceIds" type="radio" name="substances" value="{{ $result->id }}"
                class="form-radio h-4 w-4 text-green-600">
              <div class="ml-2 flex-grow">
                <div class="font-medium">{{ $result->name }}</div>
                <div class="text-sm text-gray-500">{{ $result->cas_number }} | {{ $result->stdinchikey }}</div>
              </div>
              @if (isset($result->pnec3_record_count))
                <span
                  class="ml-2 bg-green-100 px-2 py-1 rounded-full text-xs font-medium">{{ $result->pnec3_record_count }}
                  records</span>
              @endif
            </label>
          </div>
    ...
    @if ($resultsAvailable == true && count($selectedSubstances) > 0)
