<div>
            ...
            <div class="flex flex-wrap gap-2">
                @foreach ($selectedSubstances as $substance)
                    <div class="flex items-center px-4 py-2 rounded-lg bg-lime-50 border border-lime-200 shadow-sm group hover:bg-lime-100 transition duration-150 ease-in-out">
                        ...
                        @if(isset($substance['hazards_record_count']))
                            <div class="ml-3 flex items-center gap-1 px-2 py-1 rounded-full bg-lime-200 text-lime-800">
                                ...
                                <span class="text-xs font-medium">{{ $substance['hazards_record_count'] }} records</span>
                            </div>
                        @endif
    ...
    <input type="text" wire:model.live.debounce.300ms="search" name="searchSubstanceString" id="searchSubstanceString" class="w-full px-4 py-2 border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-transparent transition duration-200 ease-in-out" placeholder="Search for substances with hazards data...">
    ...
    <div class="flex flex-col md:flex-row justify-between text-sm text-gray-600 mt-2">
        <span>The search is limited to 30 substances with hazards data</span>
    ...
    <div>
        @if($resultsAvailable == true)
            <div class="mt-2 p-2 bg-lime-50 border-l-4 border-lime-500 text-lime-800 text-sm">
                <p>Select a substance and click <span class="font-semibold">Select & Search</span> to view its hazards data.</p>
            </div>
        @endif
                ...
                @foreach ($results as $result)
                    <div class="block p-2 hover:bg-gray-50 border-b border-gray-100 last:border-b-0">
                        <label class="flex items-center cursor-pointer">
                            <input wire:model.live="selectedSubstanceIds" type="radio" name="substances" value="{{ $result->id }}" class="form-radio h-4 w-4 text-green-600">
                            ...
                            @if(isset($result->hazards_record_count))
                                <span class="ml-2 bg-green-100 px-2 py-1 rounded-full text-xs font-medium">{{ $result->hazards_record_count }} records</span>
                            @endif
                        </label>
                    </div>
                @endforeach
            @elseif($search && strlen($search) > 2)
                <div class="p-4 text-center text-gray-500">No hazards substances found matching "{{ $search }}"</div>
            @else
                <div class="p-4 text-center text-gray-500">Type at least 3 characters to search</div>
            @endif
        </div>

        @if($resultsAvailable == true && $results->count() > 0)
            <div class="flex justify-end mt-4 space-x-2">
                <button type="button" wire:click="clearFilters" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition duration-150 ease-in-out">Clear</button>
                <button type="button" wire:click="applySubstanceFilter" class="btn-submit-lime">Select & Search</button>
            </div>
