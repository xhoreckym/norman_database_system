              ...
              <tbody>
                @forelse ($lifeStages as $index => $lifeStage)
                  <tr class="hover:bg-slate-300 transition {{ $index % 2 === 0 ? 'bg-slate-100' : 'bg-slate-200' }}">
                    <td class="py-2 px-4">
                      <span class="font-mono text-xs font-semibold text-gray-800 bg-gray-200 px-2 py-1 rounded">{{ $lifeStage->id }}</span>
                    </td>
                    <td class="py-2 px-4">
                    ...
                    <td class="py-2 px-4 text-center">
                      <div class="flex justify-center space-x-2">
                        ...
                        <form action="{{ route('literature.life_stages.destroy', $lifeStage) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this life stage?');">
                          @csrf
                          @method('DELETE')
