    ...
    <div class="w-full mx-auto sm:px-6 lg:px-8">
      <div class="bg-white shadow-lg sm:rounded-lg" >
        ...
        <div class="text-gray-600 flex border-l-2 border-white">
          ...
          @auth
          <div class="py-2 px-2"><a href="{{ route('arbg.gene.search.download', ['query_log_id' => $query_log_id]) }}" class="btn-submit">Download</a></div>
          @else
            ...
            <tr class="@if($loop->odd) bg-slate-100 @else bg-slate-200 @endif">
              <td class="p-1 text-center">
                <div class="flex items-center justify-center space-x-2">
                  <a href="{{ route('arbg.gene.show', $e->id) }}"
                     target="_blank"
                     class="text-teal-600 hover:text-teal-800 transition-colors"
                     title="View full record details">
                    <i class="fas fa-search"></i>
                  </a>
                  <a href="{{ route('arbg.gene.show', $e->id) }}" target="_blank" class="font-mono text-teal-800 hover:text-teal-600 hover:underline">
