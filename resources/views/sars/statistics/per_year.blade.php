...
@section('page-subtitle', 'Number of SARS-CoV-2 wastewater monitoring records per sampling year')
  ...
  @else
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <div class="text-sm text-gray-600">Average Records per Year</div>
          <div class="text-2xl font-bold text-gray-900">
            {{ number_format(collect($data)->sum() / max($totalYears, 1), 1, '.', ' ') }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @foreach(collect($data)->sortByDesc(fn($count, $year) => $year) as $year => $count)
              <tr>
