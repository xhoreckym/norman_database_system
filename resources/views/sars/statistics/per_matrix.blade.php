...
@section('page-title', 'Records by Sample Matrix')
@section('page-subtitle', 'Number of SARS-CoV-2 records per sample matrix type')
...
@section('main-content')
  ...
  @if(isset($message))
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
      ...
      <a href="{{ route('sars.statistics.index') }}" class="text-lime-600 hover:text-lime-800 underline text-sm">
  ...
  @else
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <div class="text-sm text-gray-600">Total Sample Matrices</div>
        ...
        <div>
          ...
          <div class="text-2xl font-bold text-gray-900">
            {{ number_format(collect($data)->sum(fn($item) => $item['count']) / max($totalMatrices, 1), 1, '.', ' ') }}
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @foreach(collect($data)->sortByDesc(fn($item) => $item['count']) as $matrix => $info)
              <tr>
